...
- **DNA & appearance** — `SHA-256(kind + name)` → seed → body archetype, part slot choices, palette. Emits a `CreatureAppearance` (archetype id, part ids per slot, two hex colors, species markers). Core decides what a creature looks like; it never draws it.

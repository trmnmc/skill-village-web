...
import {
  clipLeashAtBands, HOMES_HI, HOMES_LO, homesKeepOutAt, layoutHash, nearestGround, placeCreatures,
  type Spot,
} from './zones.js';
...
export function instanceSpots(
  ...
  const bands = homesKeepOutAt(anchor.y);
  const lo = Math.max(HOMES_LO, anchor.x - TETHER);
  const hi = Math.min(HOMES_HI, anchor.x + TETHER);
  ...
  [...helperIds].sort().forEach((helperId, i) => {
    ...
    const leash = Math.min(INSTANCE_LEASH, x - HOMES_LO, HOMES_HI - x);

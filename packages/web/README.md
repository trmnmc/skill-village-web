...
- **Grid renderer** — turns core's `CreatureAppearance` into a drawn creature: paint the body grid one pixel per character through the creature's color map, add the crown pixels above it, overlay eyes at the anchor coordinates so they can blink and track, and for agents swap the foot row for the tapered underside and mount the wings.
- **Motion** — the animation vocabulary from §4.2 of the spec: idle breathing, blinks, gaze, hops with anticipation and recovery, height-linked shadows, wing flap, flight-posture swaps while roaming, puff bursts, floating sleep glyphs, and pop-in speech bubbles. Every creature carries a phase offset so nothing moves in lockstep.
